...
Run the manual script `supabase/scripts/create_vector_index_after_backfill.sql` only after the embedding backfill has inserted non-null values into `knowledge_chunks.embedding`. The numbered migration `supabase/migrations/002_create_vector_index_after_backfill.sql` is intentionally kept as a no-op stub so Supabase projects that already recorded version `002` remain aligned with the local migration history. The actual index-building SQL lives outside `supabase/migrations` so `supabase db reset` and new remote deployments can apply the schema before seed or backfill steps run. The script prefers an HNSW cosine index when the installed Supabase pgvector version exposes the `hnsw` index access method. If HNSW is unavailable, it falls back to an IVFFlat cosine index; because IVFFlat depends on the data distribution at creation time, create or rebuild that fallback index after embeddings are loaded.
